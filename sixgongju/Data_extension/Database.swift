...
class DBHelper {
    ...
    func createTable(){
        ...
        let query = """
           ...
           my_question TEXT NOT NULL,
           my_complete INT DEFAULT 0
           );
           """
    ...
    func insertData(question: String) {
        // id 는 Auto increment 속성을 갖고 있기에 빼줌.
        let insertQuery = "insert into myTable (id, my_question, my_complete) values (?, ?, ?);"
        var statement: OpaquePointer? = nil
        
        if sqlite3_prepare_v2(self.db, insertQuery, -1, &statement, nil) == SQLITE_OK {
            sqlite3_bind_text(statement, 2, NSString(string: question).utf8String, -1, nil)
//            sqlite3_bind_int(statement, 3, Int32(age))                //default
    ...
    func readData() -> [MyModel] {
        ...
        while sqlite3_step(statement) == SQLITE_ROW {
            ...
            result.append(MyModel(id: Int(id), question: String(name), complete: Int(age)))
        }
    ...
    func updateData(id: Int, complete: Int) {
        var statement: OpaquePointer?
        // 등호 기호는 =이 아니라 ==이다.
        // string 부분은 작은 따옴표 두 개로 감싸줘야 한다.
        let queryString = "UPDATE myTable SET my_complete = \(complete) WHERE id == \(id)"
...
//    func completeQuestionCount() -> Int{
//        var statement: OpaquePointer?
//
//        let query = "SELECT COUNT(*) FROM MyModel WHERE complete = 1"
//
